...
const pdfUser = (datauser) => {
  ...
  datauser.forEach((element, index, array) => {
    info.push([index + 1, element.namauser, element.username, element.role]);
  });
  ...
  doc.text("Laporan User Akses", 84, 30);
  ...
  doc.autoTable({
    ...
    theme: "grid",
    startY: 40,
    head: [["No", "Nama User", "Username", "Role"]],
    body: info,
  });
  ...
  doc.save(`laporan_user_akses.pdf`);
};
